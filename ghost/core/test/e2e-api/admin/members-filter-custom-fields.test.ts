...
// Filtering members by their custom field values. Values live in a separate table
// keyed by the field's key (text in value_text, a composite address as a JSON blob
// in value_json), reached through the model's `custom_fields` relation. A filter
// names the field by key and matches on value:
...
describe('Members filtering by custom fields', function () {
    ...
    describe('negation', function () {
        ...
        it('excludes an address subfield value (is-not)', async function () {
            await createField({name: 'Shipping address', type: 'address'});
