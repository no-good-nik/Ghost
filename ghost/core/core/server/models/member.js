...
const Member = ghostBookshelf.Model.extend({
    ...
    filterRelations(options = {}) {
        return {
            // Custom field values are only filterable when the feature is on. Gating
            // the relation here (rather than the whole model) means a
            // `custom_fields.*` filter is simply an unknown relation when the flag is
            // off, and the filter is rejected — nothing to special-case downstream.
